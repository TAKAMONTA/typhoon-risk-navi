...
- **取得タイミング**: ユーザーが「現在地を追加」機能を明示的に実行した時のみ
- **保存場所**: ユーザーの端末ローカル（iOS の UserDefaults）のみ
- **送信先**: なし。外部サーバ・第三者サービスには一切送信しません
- **目的**: 台風の現在位置と保存場所との距離・到達時間予測のため
...
- アカウント・サインイン情報（本アプリにはアカウント機能がありません）
- 端末の広告識別子（IDFA）
...
すべてのユーザーデータは端末ローカルに保存されます。**アプリを iOS から削除すれば、すべてのデータが完全に削除されます**。
...
- **When collected**: Only when the user explicitly taps "Add Current Location"
- **Storage**: Locally on the user's device only (iOS UserDefaults)
- **Transmission**: None. Never sent to external servers or third parties.
- **Purpose**: To compute distance from a typhoon and arrival time predictions
...
- Account or sign-in credentials (the App has no account features)
...
All user data is stored locally on the device. **Deleting the App from iOS will permanently delete all user data**.
