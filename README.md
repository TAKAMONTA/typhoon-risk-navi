...
- **みんなタブ**: 台風の体感を沖縄10エリア（那覇 / 南部 / 中部 / 北部 / 慶良間・粟国 / 久米島 / 宮古 / 石垣 / 与那国 / 大東）ごとに共有 + 5段階のムード（おだやか / 風が出てきた / 雨風が強い / 外は危険 / 暴風）と20種の定型フレーズから選択（自由入力なし） + 直近3時間をエリア単位で集計表示（最多ムードを採用、同数時はより深刻な方） + 「公式情報ではない」旨をグリッド・詳細シート双方に明示 + 投稿には iCloud サインインが必要（閲覧のみなら不要）
...
- 個人を特定できる情報とアプリの使用状況の分析データは収集しません
- 緯度・経度そのものは端末の外に出ません。位置情報は「現在地を追加」（保存場所は端末ローカルのみに保存）と、「みんな」タブの投稿エリア自動選択の2つの用途にのみ使用します
- 「みんな」タブへの投稿だけは外部に送信されます。送信するのはエリア区分・体感レベル・定型フレーズID の3つのみで、緯度・経度や自由記述は含みません。送信先は Apple の CloudKit パブリックデータベースで、独自サーバーはありません。投稿は誰でも閲覧できる状態で保存され、投稿には iCloud サインインが必要です（閲覧は不要）
- 外部通信先は気象庁・米軍 JTWC（台風データの読み取りのみ）・Apple CloudKit（ユーザーが入力した内容を送る書き込み）の3つです
